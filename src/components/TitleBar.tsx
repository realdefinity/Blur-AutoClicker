...
export default function TitleBar({
  ...
  const activeNavIndex = TAB_ITEMS.findIndex((item) => item.value === tab);
  const activeNavColor =
    activeNavIndex >= 0 ? TAB_ITEMS[activeNavIndex].color : "var(--accent-green)";
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
        ...
        <div
          className="tab-icon-group"
          data-active-index={activeNavIndex}
          style={{ "--active-tab-color": activeNavColor } as CSSProperties}
        >
          {TAB_ITEMS.map((item) => {
